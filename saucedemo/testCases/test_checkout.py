...
from saucedemo.pageObjects.CheckoutPage import Checkoutdetails
...
class Test_Checkout():
    ...
    def test_login(self, setup):
        ...
        if cart_added_value == "Remove":
            ...
            assert False
        self.cp=Checkoutdetails(self.driver)
        self.cp.checkoutDetails()
        checkout_value = self.cp.checkout_value
        print(checkout_value)
        if checkout_value == "Checkout: Your Information":
            self.logger.info("****checkout details test passed ****")
            assert True
        else:
            self.logger.error("****checkout details test failed ****")
            assert False
